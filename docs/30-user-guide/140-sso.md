...
The behavior described above corresponds to the default `onDemand` mode, where the user decides on a per-login basis whether a session should be persisted.

Kubauth supports three SSO modes, configured globally through the `oidc.sso.mode` Helm value:

| Mode       | "Remember me" checkbox | Behavior                                                                                                        |
|------------|------------------------|-----------------------------------------------------------------------------------------------------------------|
| `onDemand` | Displayed              | An SSO session is created only when the user checks the box (the default).                                      |
| `always`   | Hidden                 | An SSO session is always created after a successful login. Subsequent logins reuse it without any user prompt.  |
| `never`    | Hidden                 | No SSO session is ever created. The user must enter credentials for every login.                                |

!!! note

    These modes apply to the internal login form. When the user authenticates through an [upstream provider](200-upstream-providers.md), an additional welcome screen is presented in `onDemand` mode to let the user decide whether to persist the session.
...
???+ abstract "values.yaml"
    ...
    oidc:
      ...
      ...
      sso:
        mode: onDemand       # always | never | onDemand
...
- `mode`: Selects the SSO behavior (`always`, `never` or `onDemand`, see the table above). Defaults to `onDemand`.
- `sessionNamespace`: Specifies the namespace where SSO sessions are persisted. Defaults to the Helm release namespace.
- `lifeTime`: Sets the session duration. This is an absolute duration with no idle timeout.
- `cleanupPeriod`: How often the controller scans for expired sessions and deletes them.
